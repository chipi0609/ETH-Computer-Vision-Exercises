...
while currentIteration <= maxIterations
   ...
   %set lambda here
   r_array=real(sqrt(dist2(Y,Y)));
   mean_dist=mean(r_array(:));
   lambda = mean_dist^2;  

   costMatrixC = chi2_cost(ShapeDescriptors1,ShapeDescriptors2);
   %costMatrixC = hist_cost_2(ShapeDescriptors1,ShapeDescriptors2);
   [corespondencesIndex T] = hungarian(costMatrixC);   
