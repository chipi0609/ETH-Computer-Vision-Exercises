...
for i = 1:length(frameValues)
    ...
    hist_current = color_histogram(round(min(max(1,meanStateAPosteriori(i,1)-0.5*WidthBB),widthFrame)),round(min(max(1,meanStateAPosteriori(i,2)-0.5*HeightBB),heightFrame)),round(min(max(1,meanStateAPosteriori(i,1)+0.5*WidthBB),widthFrame)),round(min(max(1,meanStateAPosteriori(i,2)+0.5*HeightBB),heightFrame)),frame,params.hist_bin);
